let Animate = {
  ...
  data: function() {
    return {
      informs:[{method:"createHandler",category:"wow.js", title: "" , html:"", css:"", javascript:""}
     ],  
  ...
  computed: {
    contents() {
      return this.$store.state.contents3;
    }
  },
  methods: {
    ...
    createHandler(method, category, title, htmlurl, html, css, javascript){
      ...
      axios
        .post("http://localhost:3000/Animate", {
          category:category,
          title:title,
          htmlurl:htmlurl,
          html:html,
        ...
        .then(res => {
          ...
          this.$store.commit("addContent3", res.data);
        });
    },
    deleteHandler(index) {
      let target = this.contents[index];
      console.log('delete3', target.id);
      // axios.delete(`http://localhost:3000/contents/${target.id}`).then((res) => {
      //   this.contents.splice(index, 1)
      // })
      this.$store.dispatch("CONTENT_DELETE3", { target });
    },
    updateHandler(index) {
      console.log('路徑3');
      let target = this.contents[index];
      ...
      this.$router.push({ name: "update3", params: { id: target.id } });
